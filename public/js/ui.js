...
var btoa = btoa || function(buf) {
  return new Array(buf).toString('base64');
};
...
function JPEGEncoder(quality) {
  var self = this;
	...
	function initQuantTables(sf){
      ...
      console.log("byteout", byteout.length);
      console.log("Btoa" ,btoa(byteout).length)
			var jpegDataUri = 'data:image/jpeg;base64,' + btoa(byteout);
                ...
                */
                ...
                writeFrame: function(buffer) {
                    ...
                    if (frame % 1 == 0) {
                      ...
                      /*buf8NoA = new Array();
                      var xOffset = 0;
                      for (var x=0; x < this.buf8.length; x++) {
                        if (xOffset == 3) xOffset = 0;
                        else {
                          buf8NoA.push(this.buf8[x]);
                          xOffset++;
                        }
                      }
                      console.log("length", buf8NoA.length);
                      console.log("Buffer", buf8NoA);
                      var image = {data: buf8NoA, width: 256, height: 240};
                      var compression = this.nes.encoder.encode(image, 100, false);

                      console.log('compression', 'got one');
                      var drawnImage = new Image();
                      drawnImage.src = compression;
                      */
                      var width = 320, height = 180;
                      var frameData = new Array(width * height * 4);
                      var i = 0;
                      while (i < frameData.length) {
                        frameData[i++] = 0xFF; // red
                        frameData[i++] = 0x00; // green
                        frameData[i++] = 0x00; // blue
                        frameData[i++] = 0xFF; // alpha
                      }
                      ...
                      var jpegImageData = encode(rawImageData, 50);
                      console.log(jpegImageData.data);

                      this.canvasImageData.data.set(this.buf8);
                      this.canvasContext.putImageData(this.canvasImageData, 0, 0);
                      //this.canvasContext.drawImage(drawnImage, 0, 0);
                    } else {
